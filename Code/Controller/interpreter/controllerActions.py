...
def controllerGlobal(options,values):
    if len(options) == 3:
        if not "-controlleraddr" in options:
            ...
            if int(len(existsResult)) == int(0):
                ...
                if int(len(addressResult)) == int(0):
                    cursor.execute('insert into SystemAPI (apihost,apiport,apiname,known,apikey,apitype) values (\"{0}\",{1},\"{2}\",0,"None","global");'.format(addr,port,name))
                    print("OK - New global controller added with: Address - {0}, Port - {1}, Name - {2}".format(addr,port,name))
...
def controllerList(options,values):
    if len(options) == 1:
        if options[0] == "-controllername":
            conn = sqlite3.connect('database/controllerConfiguration.db')
            cursor = conn.cursor()
            cursor.execute('select * from SystemAPI where apitype = "global";')
            results = cursor.fetchall()

            if len(results) == 0:
                print("ERROR - No configured global controllers. Use help command for more information.")
                return None
            ...
            if values[0] == "all":
                ...
                results = cursor.fetchall()
                iterator = 1
                print("Configured global controllers:")
...
def controllerTrust(options,value):
    if value[0] == "all":
        ...
        for controller in response["controllerInfo"]:
            ...
            if trusted == False:
                print("INFO - Initializating trust process on global controller: {0}".format(controller[3]))
                ...
                print("INFO - Global Controller: {} is already trusted by the Local Controller".format(controller[3]))
...
def controllerUntrust(options,value):
    if value[0] == "all":
        ...
        for controller in response["controllerInfo"]:
            ...
            if untrusted == False:
                print("INFO - Initializing untrust process on global controller: {}".format(controller[3]))
                if int(controller[2]) == 443:
                ...
                print("INFO - Global Controller {} is already untrusted by the Local Controller".format(controller[3]))
    else:
